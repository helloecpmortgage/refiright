...
export function ResultsDisplay({ results }: ResultsDisplayProps) {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      pdf.save(
        `ECP_Refinance_Report_${new Date().toISOString().slice(0, 10)}.pdf`
      );
    } catch (error) {
  ...
  return (
    <div
      ref={reportRef}
      className="mt-12 space-y-8 p-4 md:p-6 bg-card rounded-lg shadow-md"
    >
      ...
      <div className="flex justify-center mt-8 screen-only">
        <Button
          ...
          variant="outline"
          size="lg"
          className="border-primary text-primary hover:bg-primary/10 font-secondary"
        >
      ...
      <style jsx global>{`
        ...
        .screen-only {
          display: flex;
        } /* Or block, inline-block etc. as needed */
        @media print {
          .pdf-only {
            display: block;
          } /* Or flex, inline-block etc. */
          .screen-only {
            display: none;
          }
          body {
          ...
          } /* Force hide elements during canvas generation */
        }
